...
openAura.config:AddAuraWatch("alliance_cost", "The amount of cash it costs to create an alliance.", 0, 10000);
...
openAura:HookDataStream("AllyKick", function(data)
	data:SetSharedVar("alliance", "");
	data:SetSharedVar("rank", RANK_RCT);
	
	if ( IsValid(openAura.schema.alliancePanel) ) then
		openAura.schema.alliancePanel:Rebuild();
	end;
end);

openAura:HookDataStream("AllySetRank", function(data)
...
openAura.chatBox:RegisterClass("victory", "ooc", function(info)
	openAura.chatBox:Add(info.filtered, "gui/silkicons/star", info.speaker, " has achieved the ", Color(139, 174, 179, 255), info.text, " victory!");
end);
