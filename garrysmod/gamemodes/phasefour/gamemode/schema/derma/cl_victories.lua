...
function PANEL:Rebuild()
	...
	for k, v in pairs( openAura.victory:GetAll() ) do
		self.currentVictory = v;
		self.panelList:AddItem( vgui.Create("aura_Victory") );
	end;
...
vgui.Register("aura_Victories", PANEL, "DFrame");
...
function PANEL:Init()
	...
	self.victory = openAura.victories:GetPanel().currentVictory;
	...
	if ( openAura.victories:Has(self.victory.name) ) then
		self.spawnIcon:SetImage("victories/achieved");
	else
		self.spawnIcon:SetImage(self.victory.image);
	end;
	...
	function self.progressBar.Paint(progressBar)
		...
		local progress = openAura.victories:Get(self.victory.name);
...
-- Called each frame.
function PANEL:Think()
	self.progressLabel:SetText(openAura.victories:Get(self.victory.name).."/"..self.victory.maximum);
	self.progressLabel:SizeToContents();
...
vgui.Register("aura_Victory", PANEL, "DPanel");